<a class="btn
    ...
    @if ($type == 'REMOVE') btn-danger
    @elseif($type == 'INFO')
        btn-info @endif


 btn-sm"
    href="#" data-toggle="modal" data-target="#modal-confirm-{{ $type }}-{{ $id }}">
...
<div class="container" style="display: inline; padding: 0px">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="overflow: visible">
                ...
                <div class="modal-footer justify-content-between">
                    ...
                    <button type="button"
                        onclick="event.preventDefault(); document.getElementById('buttonconfirm-form-{{ $id }}').submit();"
                        @if ($type == 'REMOVE') class="btn btn-danger"
                        @elseif($type == 'INFO')
                            class="btn btn-info" @endif
                        data-dismiss="modal">
    ...
    <form id="buttonconfirm-form-{{ $id }}" action="{{ route($route) }}" method="POST"
        style="display: none;">
        @csrf
