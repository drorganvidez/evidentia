...
@section('breadcrumb')
    ...
    @if (
        \Illuminate\Support\Facades\Auth::user()->hasRole('COORDINATOR') and
            $evidence->committee->id == \Illuminate\Support\Facades\Auth::user()->coordinator->committee->id)
        <li class="breadcrumb-item"><a href="{{ route('coordinator.evidence.list.all') }}">Gestionar evidencias de
                {{ \Illuminate\Support\Facades\Auth::user()->coordinator->committee->name }}</a></li>
    @else
                    ...
                    <div class="post text-justify">
                        ...
                        @else
                            ...
                            @if (!\Carbon\Carbon::now()->gt(\Config::upload_evidences_timestamp()))
                                <x-buttonconfirm :id="$evidence->id" route="evidence.remove" title="¿Seguro?"
                                    description="Esto borrará la evidencia actual, las
