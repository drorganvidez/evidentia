                                            ...
                                            <div class="card card-light mb-2">
                                                <div class="card-header" role="tab" id="heading-{{$committee->id}}">
                                                    <h4 class="card-title mb-0">
                                                        <a class="d-block w-100 collapsed" data-toggle="collapse" href="#collapse-{{$committee->id}}" 
                                                        aria-expanded="false" aria-controls="collapse-{{$committee->id}}">
                                                            Comité de {{$committee->name}}
                                                        </a>
                                                    </h4>
                                                </div>

                                                <div id="collapse-{{$committee->id}}" class="collapse" role="tabpanel" aria-labelledby="heading-{{$committee->id}}" data-parent="#accordion">
                                                    <div class="card-body">
                        ...
                        <div class="col-lg-12 mt-3">
                            ...
                            @empty
                                <div class="callout callout-warning">
                                    <h5>Ups...</h5>
                                    <p>Parece que el coordinador de registro aún no ha creado/cargado los eventos en Eventbrite.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                                ...
                                @forelse(\App\Models\MeetingRequest::nextMeetingRequests() as $meeting)
                                    <div class="card card-light mb-2">
                                        <div class="card-header" role="tab" id="heading-{{ $meeting->id }}">
                                            <h4 class="card-title mb-0">
                                                <a class="d-block w-100 collapsed" data-toggle="collapse" href="#collapse-{{ $meeting->id }}" 
                                                aria-expanded="false" aria-controls="collapse-{{ $meeting->id }}">
                                                    {{ $meeting->title }} ({{ \Carbon\Carbon::parse($meeting->datetime)->day }} de {{ \Carbon\Carbon::parse($meeting->datetime)->monthName }})
                                                </a>
                                            </h4>
                                        </div>

                                        <div id="collapse-{{ $meeting->id }}" class="collapse" role="tabpanel" aria-labelledby="heading-{{ $meeting->id }}" data-parent="#accordion">
                                            <div class="card-body">
                                                <p>
                                                    Programado para el
                ...
                @if($datetime)
                    <li class="list-group-item d-flex justify-content-between align-items-center px-1 py-1">
                        ...
                        <span>
                            ...
                            <small class="text-muted">a las {{ \Carbon\Carbon::parse($datetime)->format('H:i') }}</small>
                        </span>
                    </li>
                @endif
            @endforeach
